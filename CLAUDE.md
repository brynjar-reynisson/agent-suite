...
```bash
./build.sh          # build JAR + restart dev servers (build.cmd on Windows)
./promote.sh        # freeze JAR to release/, build frontend dist/, restart prod (promote.cmd on Windows)
./mvnw test         # run tests
```

**Dev:** frontend `http://localhost:5177`, backend `http://localhost:8090`  
**Prod:** frontend `http://localhost:5176` → `https://agent.breynisson.org`, backend `http://localhost:8091`

## Detailed Documentation

| Topic | File |
|-------|------|
| Build commands, env vars, auto-start | [docs/dev/build-and-run.md](docs/dev/build-and-run.md) |
| Database migrations, backup & restore, message types | [docs/dev/database.md](docs/dev/database.md) |
| Architecture, key components, model config, logging | [docs/dev/architecture.md](docs/dev/architecture.md) |
| API endpoints, supported models | [docs/dev/api.md](docs/dev/api.md) |
| Frontend setup, key files | [docs/dev/frontend.md](docs/dev/frontend.md) |

## Architecture at a Glance

Spring Boot 3.5 + LangChain4j 1.16.2. Java 21.
